...
trait Requests {
  ...
  def getClickId(request: Request): Option[String] = {
    var clickidCookie = request.cookies.get(Play.configuration.getProperty("click.cookie.key"))
    Logger.debug("clickidcookie:" + clickidCookie)
    if (Checks.isExistCookie(clickidCookie)) {
      Logger.debug("clickid:" + clickidCookie.value)
      return Some(clickidCookie.value)
    }
    Logger.debug("clickid:none")
    return None
  }
  /**
  ...
  def makeHeaders(adr: AdRequest,contentType: Option[String]): Map[String, String] = {
    ...
    return Map[String, String](
      ...
      "Accept" -> getHeader("accept").getOrElse(""),
      "User-Agent" -> adr.ua.getOrElse(""),
      "Accept-Charset" -> getHeader("accept-charset").getOrElse(""),
      "Referer" -> getHeader("referer").getOrElse(""),
   ...
   * @return キャリア名
   */
  def getCarrierFromIp(ip:String):Option[String] = {
